...
def remove_outlier(df, threshold=3, debug=False):
    outliers = []
    for variable in df.columns:
        z = np.abs(stats.zscore(pd.to_numeric(df[variable].values)))
    ...
    for i in range(len(outliers)):
        df.drop(index=outliers[i], inplace=True)
...
def show_cor_heatmap(df_cor, n, x_col, y_col, symmetric=False, return_pvalue=False):
    ...
    if symmetric:
        mask = np.triu(np.ones_like(data_p, dtype=bool))
        sns.heatmap(data_p, annot=True, mask=mask, linewidths=.5,
                    annot_kws={'size': 15}, cmap='seismic')
    else:
        sns.heatmap(data_p, annot=True, linewidths=.5,
                    annot_kws={'size': 15}, cmap='seismic')
...
def PCA_2(data):
    ...
    PCs = pd.DataFrame(data=principalComponents, columns=[
...
def K_means(x, y, n, plot, xlabel=None, ylabel=None):
    ...
    if (plot):
        ...
        plt.scatter(df['x'], df['y'], color=colors,
                    alpha=0.6, edgecolor=colors, s=15)
